...
namespace gps {
	void Model3D::LoadModel(std::string fileName) {
		boundingBox.minPos = glm::vec3(std::numeric_limits<float>::infinity());
		boundingBox.maxPos = glm::vec3(-std::numeric_limits<float>::infinity());
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
				...
				for (size_t v = 0; v < fv; v++) {
					...
					boundingBox.minPos.x = std::min(boundingBox.minPos.x, vx);
					boundingBox.minPos.y = std::min(boundingBox.minPos.y, vy);
					boundingBox.minPos.z = std::min(boundingBox.minPos.z, vz);

					boundingBox.maxPos.x = std::max(boundingBox.maxPos.x, vx);
					boundingBox.maxPos.y = std::max(boundingBox.maxPos.y, vy);
					boundingBox.maxPos.z = std::max(boundingBox.maxPos.z, vz);
			...
			this->boundingBox.size = glm::abs(this->boundingBox.maxPos - this->boundingBox.minPos);
			this->boundingBox.origin = (this->boundingBox.maxPos + this->boundingBox.minPos) * 0.5f;
